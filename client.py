    ...
    def run(self):
        ...
        while True:
            ...
            if message:
                ...
                if partition[0] != 'Server':
                    username = partition[0].lower()
                    b64 = json.loads(partition[2][1:])
                    iv = b64['iv']
                    cipher = b64['cipher']
                    rsa = b64decode(b64['rsa'])
                    key = RSAEncryption.decrypt_session_key(username, rsa)
                    dec_message = AESEncryption.decrypt_with_key(iv, cipher, key)
                    final_msg = partition[0] + ": " + dec_message

                    if self.messages:
                        self.messages.insert(tk.END, final_msg)
                        print('\r{}\n{}: '.format(final_msg, self.name), end = '')
...
class Client:
    ...
    def start(self):
        ...
        # Generate RSA Keys
        RSAEncryption.generate_key(self.name)
    ...
    def send(self, text_input):
        ...
        enc_key, key = RSAEncryption.get_session_key(self.name.lower())
        iv, cipher = AESEncryption.encrypt_with_key(message, key)

        message_enc = json.dumps({'rsa': b64encode(enc_key).decode('utf-8'), 'iv': iv, 'cipher': cipher})
        ...
        # Send message to server for broadcasting
            ...
            self.sock.sendall('{}: {}'.format(self.name, message_enc).encode('utf-8'))
